...
int main( int argc, char **argv )
{
    ...
    std::string pts_location_file = argv[2];
	...
	osg::Vec2i cap_size;
	cap_size.x() = atoi(argv[3]);
	cap_size.y() = atoi(argv[4]);

    // The z_lift_value of the camera.
	float z_lift = atof(argv[5]);

    std::string output_path = argv[6];

    // The desired image format.
	std::string image_format = argv[7];
    std::cout<<"image_format "<<image_format<<std::endl;


    // for 循环
    std::ifstream in(pts_location_file);
    std::string lineStr;
    ...
    if(in) // 有该文件
    {
        ...
        while (getline (in, lineStr)) // line中不包括每行的换行符
        { 
            ...
            char *p; // 分隔后的字符串
            std::vector<char *> id_lon_lat_alt;
            p = strtok(lineCharArray, SEPARATOR); // 按照spaceChar分隔
            // if(p){
            //     point_id = p;
            // }
            // p = strtok(lineCharArray, SEPARATOR); 
            while(p){
               id_lon_lat_alt.push_back(p);//#atof(p)
               p = strtok(NULL, SEPARATOR); 
            }

            int point_id = atoi(id_lon_lat_alt[0]);
            osg::Vec2 pos_lat_lon;
            pos_lat_lon.x() = atof(id_lon_lat_alt[1]);
            pos_lat_lon.y() = atof(id_lon_lat_alt[2]);

            
            //////////////////////////////////////////////////////////////////////////
            // Render images.
            // ImageCapturer::GridTileExtent(ext_min, ext_max)
            ImageCapturer::CapturingCondition condition(id_lon_lat_alt[0], pos_lat_lon, cap_size, z_lift, output_path, image_format);
            ...
            // std::cout<<"before set capture "<<std::endl;
            // capturer.SetCapturCondition(condition);
            // std::cout<<"set capturing "<<std::endl;
            //有问题无法设置
        ...
        std::cout<<"finish capturer "<<std::endl;
	    return EXIT_SUCCESS;
